...
$rankorder = (int) $_GET["rankorder"];

if ($action == "delete") {
	/* first, NULL all ranking FKs for items that use this rank. */
	$query = "UPDATE {$OPT["table_prefix"]}items SET ranking = NULL WHERE ranking = $ranking and group_id = " . $_SESSION["group_id"] ;
	mysql_query($query) or die("Could not query: " . mysql_error());
	$query = "DELETE FROM {$OPT["table_prefix"]}ranks WHERE ranking = $ranking and group_id = " . $_SESSION["group_id"];
	mysql_query($query) or die("Could not query: " . mysql_error());
	header("Location: " . getFullPath("ranks.php?message=Rank+deleted."));
	exit;
}
else if ($action == "promote") {
	$query = "UPDATE {$OPT["table_prefix"]}ranks SET rankorder = rankorder + 1 WHERE rankorder = " . $rankorder - 1 . " and group_id = " . $_SESSION["group_id"];
	mysql_query($query) or die("Could not query: " . mysql_error());
	$query = "UPDATE {$OPT["table_prefix"]}ranks SET rankorder = rankorder - 1 WHERE ranking = $ranking and group_id = " . $_SESSION["group_id"];
	mysql_query($query) or die("Could not query: " . mysql_error());
	...
	$query = "SELECT title, rendered FROM {$OPT["table_prefix"]}ranks WHERE ranking = $ranking and group_id = " . $_SESSION["group_id"];
	...
	if (!$haserror) {
		$query = "UPDATE {$OPT["table_prefix"]}ranks " .
					...
					"WHERE ranking = $ranking and group_id = " . $_SESSION["group_id"];
		mysql_query($query) or die("Could not query: " . mysql_error());
...
$query .= " WHERE group_id = " . $_SESSION["group_id"] . 
