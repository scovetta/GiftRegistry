						...
						<tr class="<?php echo (!($i++ % 2)) ? "evenrow" : "oddrow" ?>" valign="top">
							<td>
								...
								if ($row["image_filename"] != "" && $OPT["allow_images"]) {
									?>
									<a href="javascript:void(0);" onMouseOver="return overlib('<img src=\'<?php echo $OPT["image_subdir"] . "/" . $row["image_filename"]; ?>\' />', STICKY, MOUSEOFF);" onMouseOut="return nd();"><img src="images/image_obj.gif" border="0" alt="Image"></a>
									<?php	
								}
								?>
							</td>
					...
					while ($row = mysql_fetch_array($shoppees,MYSQL_ASSOC)) {
						...
						<tr class="<?php echo (!($i++ % 2)) ? "evenrow" : "oddrow" ?>">
							<td>
								...
								if ($row["comment"] != "") {
									?>
									<a href="javascript:void(0);" onMouseOver="return overlib('<?php echo fixForJavaScript($row["comment"]); ?>', STICKY, MOUSEOFF);" onMouseOut="return nd();"><img src="images/view.gif" alt="<?php echo htmlspecialchars($row["comment"]); ?>" border="0"></a>
									<?php
								}
								?>
							</td>
