...
if (isset($_GET["eventid"]) && $_GET["eventid"] != "") {
	$query = "SELECT * FROM {$OPT["table_prefix"]}events WHERE eventid = " . $_GET["eventid"] . " AND group_id = " . $_SESSION["group_id"] . " AND ";
	if ($_SESSION["admin"] == 1)
...
if ($action == "delete") {
	$query = "DELETE FROM {$OPT["table_prefix"]}events WHERE eventid = " . $_GET["eventid"] . " AND group_id = " . $_SESSION["group_id"];
	mysql_query($query) or die("Could not query: " . mysql_error());
	header("Location: " . getFullPath("event.php?message=Event+deleted."));
	exit;
}
else if ($action == "edit") {
	$query = "SELECT description, eventdate, recurring, userid FROM {$OPT["table_prefix"]}events WHERE eventid = " . $_GET["eventid"] . " AND group_id = " . $_SESSION["group_id"];
	$rs = mysql_query($query) or die("Could not query: " . mysql_error());
	...
	if (!$haserror) {
		$query = "INSERT INTO {$OPT["table_prefix"]}events(userid,group_id,description,eventdate,recurring) " .
					"VALUES(" . ($systemevent ? "NULL" : $userid) . "," . $_SESSION["group_id"]. ",'$description','" . strftime("%Y-%m-%d",$eventdate) . "',$recurring)";
		mysql_query($query) or die("Could not query: " . mysql_error());
	...
	if (!$haserror) {
		$query = "UPDATE {$OPT["table_prefix"]}events SET " .
				...
				"recurring = $recurring " . 
				"WHERE eventid = " . $_GET["eventid"] . " and group_id = " . $_SESSION["group_id"];
		mysql_query($query) or die("Could not query: " . mysql_error());
...
$query = "SELECT eventid, userid, description, eventdate, recurring " .
			...
			"WHERE group_id = " . $_SESSION["group_id"] . " AND ";

if ($_SESSION["admin"] == 1)
	$query .= " (userid = $userid OR userid IS NULL)";		// add in system events
else 
	$query .= " userid = $userid ";
