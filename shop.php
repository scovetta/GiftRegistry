...
if (!empty($_GET["action"])) {
	...
	if ($action == "reserve") {
		...
		$query = "INSERT INTO items(userid,description,price,source,url,category) SELECT $userid, description, price, source, url, category FROM items WHERE itemid = " . (int) $_GET["itemid"] . " and group_id = " . $_SESSION["group_id"];
		mysql_query($query) or die("Could not query: " . mysql_error());
		*/
		/* TODO: copy the image too? */
		$query = "SELECT userid, description, price, source, url, category, comment FROM {$OPT["table_prefix"]}items WHERE itemid = " . (int) $_GET["itemid"] . " and group_id = " . $_SESSION["group_id"];
		...
		$query = "INSERT INTO {$OPT["table_prefix"]}items(userid,group_id,description,price,source,url,comment,category,ranking,quantity) VALUES($userid," . $_SESSION["group_id"] . ",'$desc','$price','$source'," . (($url == "") ? "NULL" : "'$url'") . "," . (($comment == "") ? "NULL" : "'$comment'") . "," . (($cat == "") ? "NULL" : $cat) . ",1,1)";
...
$rs = mysql_query("SELECT * FROM {$OPT["table_prefix"]}shoppers WHERE shopper = $userid AND mayshopfor = $shopfor AND pending = 0 and group_id = " . $_SESSION["group_id"]) or die("Could not query: " . mysql_error());
...
$query = "SELECT i.itemid, description, price, source, c.category, url, image_filename, " .
	...
	"LEFT OUTER JOIN {$OPT["table_prefix"]}categories c ON c.categoryid = i.category AND c.group_id = i.group_id " .
	"LEFT OUTER JOIN {$OPT["table_prefix"]}ranks r ON r.ranking = i.ranking and r.group_id = i.group_id " .
	"LEFT OUTER JOIN {$OPT["table_prefix"]}allocs a ON a.itemid = i.itemid AND i.quantity = 1 and a.group_id = i.group_id " .	// only join allocs for single-quantity items.
	"LEFT OUTER JOIN {$OPT["table_prefix"]}users ub ON ub.userid = a.userid AND a.bought = 1 and ub.group_id = a.group_id " .
	"LEFT OUTER JOIN {$OPT["table_prefix"]}users ur ON ur.userid = a.userid AND a.bought = 0 and ur.group_id = a.group_id " .
	"WHERE i.userid = $shopfor and a.group_id = " . $_SESSION["group_id"] . " AND a.group_id = i.group_id and a.group_id = ub.group_id and a.group_id = ur.group_id and a.group_id = r.group_id ORDER BY $sortby";
$rs = mysql_query($query) or die("Could not query: " . mysql_error());
...
/* okay, I *would* retrieve the shoppee's fullname from the items recordset,
	...
	hit the DB again. */
$query = "SELECT fullname FROM {$OPT["table_prefix"]}users WHERE userid = $shopfor and group_id = " . $_SESSION["group_id"];
...
<script language="JavaScript" type="text/javascript" src="overlib/overlib.js"><!-- overLIB (c) Erik Bosrup --></script>
	...
	while ($row = mysql_fetch_array($rs,MYSQL_ASSOC)) {
		...
		<tr class="<?php echo (!($i++ % 2)) ? "evenrow" : "oddrow" ?>" valign="top">	
			...
			<td>
				...
				if ($comment != "") {
					?>
					<a href="javascript:void(0);" onMouseOver="return overlib('<?php echo fixForJavaScript($row["comment"]); ?>', STICKY, MOUSEOFF);" onMouseOut="return nd();"><img src="images/topic.gif" border="0" alt="<?php echo htmlspecialchars($row["comment"]); ?>"></a>
					<?php
				}
				if ($image_filename != "" && $OPT["allow_images"]) {
					?>
					<a href="javascript:void(0);" onMouseOver="return overlib('<img src=\'<?php echo $OPT["image_subdir"] . "/" . $image_filename; ?>\' />', STICKY, MOUSEOFF);" onMouseOut="return nd();"><img src="images/image_obj.gif" border="0" alt="Image"></a>
					<?php
				}
				?>
			</td>
			...
			if ($quantity > 1) {
				...
				$query = "SELECT a.quantity, a.bought, a.userid, " .
						...
						"LEFT OUTER JOIN {$OPT["table_prefix"]}users ub ON ub.userid = a.userid AND a.bought = 1 AND ub.group_id = a.group_id " .
						"LEFT OUTER JOIN {$OPT["table_prefix"]}users ur ON ur.userid = a.userid AND a.bought = 0 AND ur.group_id = a.group_id " .
						"WHERE a.itemid = $itemid and a.group_id = " . $_SESSION["group_id"] .
