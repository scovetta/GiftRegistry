...
$query = "SELECT quantity FROM {$OPT["table_prefix"]}items WHERE itemid = $itemid and group_id = " . $_SESSION["group_id"];
...
if ($quantity == 1) {
	...
	$query = "DELETE FROM {$OPT["table_prefix"]}allocs WHERE itemid = $itemid and group_id = " . $_SESSION["group_id"];
	mysql_query($query) or die("Could not query: " . mysql_error());
	$query = "DELETE FROM {$OPT["table_prefix"]}items WHERE itemid = $itemid and group_id = " . $_SESSION["group_id"];
	mysql_query($query) or die("Could not query: " . mysql_error());
	...
	if ($actual == $quantity) {
		...
		$query = "UPDATE {$OPT["table_prefix"]}items SET quantity = quantity - $actual WHERE itemid = $itemid and group_id = " . $_SESSION["group_id"];
	}
			...
			<TR valign="top">
				...
				<TD>
					...
					$query = "SELECT u.userid, u.fullname " .
									...
									"INNER JOIN {$OPT["table_prefix"]}users u ON u.userid = s.shopper and u.group_id = s.group_id " .
