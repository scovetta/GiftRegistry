...
if ($action == "delete") {
	...
	$query = "UPDATE {$OPT["table_prefix"]}items SET category = NULL WHERE category = " . addslashes($_GET["categoryid"]) . " and group_id = " . $_SESSION["group_id"];
	mysql_query($query) or die("Could not query: " . mysql_error());
	$query = "DELETE FROM {$OPT["table_prefix"]}categories WHERE categoryid = " . addslashes($_GET["categoryid"]) . " and group_id = " . $_SESSION["group_id"];
	mysql_query($query) or die("Could not query: " . mysql_error());
	header("Location: " . getFullPath("categories.php?message=Category+deleted."));
	exit;
}
else if ($action == "edit") {
	$query = "SELECT category FROM {$OPT["table_prefix"]}categories WHERE categoryid = " . $_GET["categoryid"] . " and group_id = " . $_SESSION["group_id"];
	...
	if (!$haserror) {
		$query = "INSERT INTO {$OPT["table_prefix"]}categories(categoryid,group_id,category) " .
					"VALUES(NULL,$_SESSION["group_id"],'$category')";
		mysql_query($query) or die("Could not query: " . mysql_error());
	...
	if (!$haserror) {
		$query = "UPDATE {$OPT["table_prefix"]}categories " .
					...
					"WHERE categoryid = " . addslashes($_GET["categoryid"]) . " and group_id = " . $_SESSION["group_id"];
		mysql_query($query) or die("Could not query: " . mysql_error());
