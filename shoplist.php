...
$query = "SELECT description, source, price, i.comment, a.quantity, a.quantity * i.price AS total, rendered, fullname " .
			...
			"INNER JOIN {$OPT["table_prefix"]}users u ON u.userid = i.userid AND u.group_id = i.group_id " .
			"INNER JOIN {$OPT["table_prefix"]}ranks r ON r.ranking = i.ranking AND r.group_id = i.group_id " .
			"INNER JOIN {$OPT["table_prefix"]}allocs a ON a.userid = $userid AND a.itemid = i.itemid AND bought = 0 AND a.group_id = i.group_id " .
            "WHERE i.group_id = " . $_SESSION["group_id"] . " AND i.group_id=a.group_id and i.group_id=u.group_id and i.group_id=r.group_id " .
			"ORDER BY $sortby";
