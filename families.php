...
if ($action == "delete") {
	...
	$query = "DELETE FROM {$OPT["table_prefix"]}memberships WHERE familyid = " . addslashes($_GET["familyid"]) . " AND group_id = " . $_SESSION["group_id"];
	mysql_query($query) or die("Could not query: " . mysql_error());
	$query = "DELETE FROM {$OPT["table_prefix"]}families WHERE familyid = " . addslashes($_GET["familyid"]) . " AND group_id = " . $_SESSION["group_id"];
	mysql_query($query) or die("Could not query: " . mysql_error());
	header("Location: " . getFullPath("families.php?message=Family+deleted."));
	exit;
}
else if ($action == "edit") {
	$query = "SELECT familyname FROM {$OPT["table_prefix"]}families WHERE familyid = " . $_GET["familyid"] . " AND group_id = " . $_SESSION["group_id"];
	...
	if (!$haserror) {
		$query = "INSERT INTO {$OPT["table_prefix"]}families(familyid,group_id,familyname) " .
					"VALUES(NULL," . $_SESSION["group_id"] . ",'$familyname')";
		mysql_query($query) or die("Could not query: " . mysql_error());
	...
	if (!$haserror) {
		$query = "UPDATE {$OPT["table_prefix"]}families " .
					...
					"WHERE familyid = " . addslashes($_GET["familyid"]) . " AND group_id = " . $_SESSION["group_id"];
		mysql_query($query) or die("Could not query: " . mysql_error());
	...
	foreach ($members as $userid) {
		$query = "INSERT INTO {$OPT["table_prefix"]}memberships(userid,familyid,group_id) VALUES(" . addslashes($userid) . "," . addslashes($_GET["familyid"]) . "," . $_SESSION["group_id"] . ")";
		mysql_query($query) or die("Could not query: " . mysql_error());
	}
...
$query = "SELECT f.familyid, familyname, COUNT(userid) AS members " .
			...
			" LEFT OUTER JOIN {$OPT["table_prefix"]}memberships m ON m.familyid = f.familyid and m.group_id = f.group_id " .
            " WHERE m.group_id = " . $_SESSION["group_id"] AND f.group_id = " . $_SESSION["group_id"] .
			" GROUP BY f.familyid " .
			" ORDER BY familyname";
